...
def export_to_excel(data, filename):
    ...
    try:
        ...
        for row in range(2, sheet.max_row + 1):
            ...
            existing_rows[(net_name, scheme, budget, neuron_selection)] = row
        
        # Write or overwrite data
        for row_data in data:
            net_name, scheme, budget, neuron_selection, valid_best, valid_best_epoch, test_top1_valid_best, best_epoch_mean = row_data
            key = (net_name, scheme, budget, neuron_selection)
            if key in existing_rows:
                row_index = existing_rows[key]
                # sheet.cell(row=row_index, column=3).value = budget
                sheet.cell(row=row_index, column=5).value = valid_best
                sheet.cell(row=row_index, column=6).value = valid_best_epoch
                sheet.cell(row=row_index, column=7).value = test_top1_valid_best
                sheet.cell(row=row_index, column=8).value = best_epoch_mean
            else:
        ...
        wb = Workbook()
        ws = wb.active
        ws.append(["Model", "Scheme", "Budget", "Method", "valid/best", "valid/best at epoch", "test/top1 at valid/best", "Average Stopping Epoch"])
        for row_data in data:
            ws.append(row_data)
        wb.save(filename)
